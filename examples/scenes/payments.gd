...
onready var is_payments_available_label = $MarginContainer2/VBoxContainer/IsPaymentsAvailable
...
func _ready():
	...
	is_payments_available_label.text = "Is Payments Available: " + str(Bridge.payments.is_available)
...
func _on_purchase_completed(success, purchase):
	...
	if purchase != null:
		match Bridge.platform.id:
			"yandex":
				print("Purchase ID: " + str(purchase.productID))

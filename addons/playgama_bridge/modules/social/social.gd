...
var is_post_reward_supported setget , _is_post_reward_supported_getter
...
var _js_get_post_reward_then = JavaScript.create_callback(self, "_on_js_get_post_reward_then")
var _js_get_post_reward_catch = JavaScript.create_callback(self, "_on_js_get_post_reward_catch")
...
func get_post_reward(callback = null):
	...
	_js_social.getPostReward() \
		.then(_js_get_post_reward_then) \
		.catch(_js_get_post_reward_catch)
		...
		match data_type:
			TYPE_OBJECT:
				...
				for i in range(data.length):
					...
					array.append(item)
				_get_post_reward_callback.call_func(true, array)
			_:
				_get_post_reward_callback.call_func(false, [])
